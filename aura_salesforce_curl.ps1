...
function Invoke-AuraExploit {
    ...
    $tmpBody = Join-Path $env:TEMP ("aura_body_" + (Get-Random) + ".txt")
    $tmpOut  = Join-Path $env:TEMP ("aura_out_" + (Get-Random) + ".txt")
    $tmpCode = Join-Path $env:TEMP ("aura_code_" + (Get-Random) + ".txt")

    # Set-Content peut ajouter une newline selon versions; ici c'est acceptable dans la majorité des cas.
    # Si tu veux du byte-perfect strict: lance depuis PowerShell 7 avec Set-Content -NoNewline.
    try {
        Set-Content -Path $tmpBody -Value $postBodyString -Encoding UTF8 -NoNewline
    }
    catch {
        Set-Content -Path $tmpBody -Value $postBodyString -Encoding UTF8
    }
    ...
    if ($exitCode -ne 0) {
        Remove-Item -Path $tmpBody,$tmpOut -Force -ErrorAction SilentlyContinue
